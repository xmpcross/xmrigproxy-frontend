...
$$(document).on('page:init', '.page[data-name="main"]', function (e) {
	...
	$$('#change_pool').on("change", function(e) {
...
function get_data(){
	app.request.post('php/get_json.php', {cc:"proxy_data",  endpoint: 'summary', proxy: proxy_id}, function (data) {
        ...
        var pool_list = "";
        $.each(data.config_data.pools, function(i, item) {
            pool_list+='<option value="'+i+'" ' + (i == 0 ? "selected" : "") + '>'+item.url+'</option>';
        });
		$$("#change_pool").html(pool_list);
